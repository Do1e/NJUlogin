...
class CaptchaOCR():
    def __init__(self, gpu_id: int = -1, use_import_onnx: bool = False,
        ...
        if self.use_import_onnx:
            if import_onnx_path is None:
                raise ValueError('import_onnx_path must be specified when use_import_onnx is True')
            ...
            if charsets_path is None:
                raise ValueError('charsets_path must be specified when use_import_onnx is True')
        ...
        self.ort_session = onnxruntime.InferenceSession(import_onnx_path, providers=providers)
    ...
    def get_text(self, img: Union[bytes, str, pathlib.PurePath, Image.Image]):
        if not isinstance(img, (bytes, str, pathlib.PurePath, Image.Image)):
            raise TypeError('img must be bytes, str, pathlib.PurePath or PIL.Image.Image')
        ...
        if self.resize[0] == -1:
            if self.word:
                ...
                size = (int(image.size[0] * self.resize[1] / image.size[1]), int(self.resize[1]))
        else:
